...
from didit.schemas.common import CallbackMethodEnum, LanguageEnum, SandboxScenarioEnum, StatusEnum

    
class CreateSessionRequest(BaseModel):
    workflow_id: str #id of the workflow to be executed (required)
    callback:HttpUrl | None = None #callback url for the session (optional)
    ...
    contact_details:EmailStr | str | None = None #Email address or phone number of the user. (optional)
    
    expected_details:dict | None = None #Expected details of the user. (optional)
...
class CreateSessionResponse(BaseModel):
    ...
    session_number: str #Human-readable sequence number shown in the Didit console.
    ...
    url: HttpUrl #Hosted verification URL to redirect the user to, or embed in an iframe. (Field name is url, not verification_url.) When the request includes language, the URL contains a language path segment — e.g. https://verify.didit.me/en/session/3FaJ9wLqX2Mz.
    ...
    workflow_version: str #Published workflow version the session was pinned to at creation.
